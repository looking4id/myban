    ...
    <div className="flex flex-col items-center">
      <div className="relative w-20 h-20 flex items-center justify-center">
        <svg className="transform -rotate-90 w-full h-full">
          ...
          <circle cx="40" cy="40" r={radius} stroke={color} strokeWidth="6" fill="transparent" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} strokeLinecap="round" />
        </svg>
        ...
        <div className="flex items-center gap-4">
            <div className="relative w-16 h-16 flex items-center justify-center">
                <div className="absolute inset-0 rounded-full border-[5px] border-slate-100"></div>
                 {total > 0 && <div className={`absolute inset-0 rounded-full border-[5px] border-transparent border-t-${colorClass} border-r-${colorClass} rotate-45`}></div>}
...
export const PriorityBadge = ({ priority }: { priority?: Priority }) => {
    ...
    if (priority === Priority.High) {
      return <span className={`${baseClass} border-rose-100 text-rose-600 bg-rose-50`}><div className="w-1.5 h-1.5 rounded-full bg-rose-600"></div>高</span>;
    }
    if (priority === Priority.Normal) {
      return <span className={`${baseClass} border-orange-100 text-orange-600 bg-orange-50`}><div className="w-1.5 h-1.5 rounded-full bg-orange-600"></div>中</span>;
    }
    return <span className={`${baseClass} border-slate-200 text-slate-500 bg-slate-50`}><div className="w-1.5 h-1.5 rounded-full bg-slate-400"></div>低</span>;
};
...
export const SeverityBadge = ({ severity }: { severity?: Severity }) => {
    ...
    switch (severity) {
        case Severity.Critical: return <span className={`${baseClass} bg-red-600 text-white shadow-sm shadow-red-200 animate-pulse`}>致命</span>;
        case Severity.Major: return <span className={`${baseClass} bg-orange-500 text-white shadow-sm shadow-orange-100`}>严重</span>;
        case Severity.Normal: return <span className={`${baseClass} bg-blue-500 text-white shadow-sm shadow-blue-100`}>一般</span>;
        case Severity.Minor: return <span className={`${baseClass} bg-slate-100 text-slate-400 border border-slate-200`}>提示</span>;
        default: return <span className={`${baseClass} bg-slate-50 text-slate-400`}>-</span>;
    }
};
