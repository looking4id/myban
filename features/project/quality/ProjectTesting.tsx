...
import { TestCaseDetailsDrawer, TestPlanDetailsDrawer, TestCreateModal } from './SharedComponents';
import { TestCase, TestReview as TestReviewType, TestPlan as TestPlanType, TestReport as TestReportType, ReviewResult } from './types';

export const ProjectTesting = () => {
  const [activeSubItem, setActiveSubItem] = useState('测试用例脑图');
  ...
  /**
   * Added helper function to transform mindmap node click events into selected test cases
   */
  const handleMindNodeClick = (node: any) => {
    const tc = testCases.find(t => t.title === node.title) || {
      id: node.id,
      title: node.title,
      version: 'v1.0',
      reviewStatus: '待评审',
    ...
    setSelectedTestCase(tc as TestCase);
  };
    ...
    <div className="bg-white rounded-none border border-slate-200 shadow-sm h-full flex overflow-hidden font-sans relative">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white relative">
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          {activeSubItem === '测试用例脑图' && <TestCaseMindmap onNodeClick={handleMindNodeClick} />}
