...
import { ChevronDown, RefreshCw } from '../../components/common/Icons';
...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-[#f9fafb] text-slate-800 font-sans">
      <div className="bg-white rounded shadow-2xl shadow-slate-200/50 w-[960px] h-[580px] flex overflow-hidden">
        ...
        <div className="w-[55%] p-12 flex flex-col justify-center relative bg-white">
            ...
            <form onSubmit={handleLogin} className="space-y-5">
                {error && <div className="text-xs text-red-500 bg-red-50 px-3 py-2 rounded border border-red-100">{error}</div>}
                {loginMethod === 'phone' ? (
                    <>
                        <div className="space-y-2">
                            ...
                            <div className="flex items-center border border-slate-300 rounded hover:border-blue-500 transition-colors focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 h-[42px] overflow-hidden">
                                ...
                                <input type="text" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="输入手机号" className="flex-1 h-full px-3 text-sm outline-none text-slate-700 placeholder:text-slate-300" />
                            </div>
                        </div>
                        <div className="space-y-2">
                            ...
                            <div className="flex items-center border border-slate-300 rounded hover:border-blue-500 transition-colors focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 h-[42px] overflow-hidden bg-white">
                                <input type="text" value={code} onChange={(e) => setCode(e.target.value)} placeholder="输入手机验证码" className="flex-1 h-full px-3 text-sm outline-none text-slate-700 placeholder:text-slate-300" />
                                <button type="button" onClick={handleSendCode} disabled={countdown > 0} className="px-4 py-2 mr-1 rounded bg-slate-50 text-slate-500 text-xs hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2">
                    ...
                    <>
                        <div className="space-y-2">
                            ...
                            <div className="flex items-center border border-slate-300 rounded hover:border-blue-500 transition-colors focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 h-[42px] overflow-hidden">
                                <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="请输入用户名/手机号/邮箱" className="flex-1 h-full px-3 text-sm outline-none text-slate-700 placeholder:text-slate-300" />
                            </div>
                        </div>
                         <div className="space-y-2">
                            <label className="text-slate-500 text-sm">密码</label>
                            <div className="flex items-center border border-slate-300 rounded hover:border-blue-500 transition-colors focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 h-[42px] overflow-hidden">
                                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="请输入密码" className="flex-1 h-full px-3 text-sm outline-none text-slate-700 placeholder:text-slate-300" />
                            </div>
                        </div>
                    </>
                )}
                <button type="submit" disabled={loading} className="w-full bg-[#528eff] hover:bg-blue-600 text-white h-[42px] rounded text-sm font-medium transition-colors flex items-center justify-center gap-2 mt-6 shadow-sm shadow-blue-200">
                    {loading && <RefreshCw size={16} className="animate-spin" />} 登录
                </button>
            </form>
      ...
      <div className="mt-8 text-xs text-slate-400 font-sans">© PingCode</div>
    </div>
  );
};
